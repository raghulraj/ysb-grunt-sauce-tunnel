		...
		grunt.registerMultiTask('ysb_sauce_tunnel', 'Runs the Sauce Labs tunnel', function () {
			...
			function start(options) {
				...
				tunnel = new SauceTunnel(
					...
					options.identifier,
					true, // tunneled = true
					['-v','--doctor','--pac',options.pac,'-B', 'mint.qa.cart-secure.com,mint.stage.cart-secure.com','-X', options.scproxy, '-P', options.seport]
					);
				}
				else
				{
					tunnel = new SauceTunnel(
                                        ...
                                        options.identifier,
                                        true, // tunneled = true
					['-v','--doctor','-X', options.scproxy, '-P', options.seport]
                                        );
				}
